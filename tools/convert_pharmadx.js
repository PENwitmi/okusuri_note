...
class PharmaDxConverter {
    constructor() {
        ...
        this.individualDrugs = new Map(); // 個別薬剤MDファイル用
        ...
        this.protectionSettings = {
            minLines: 100,           // 保護対象の最小行数
            detailedContentMarkers: [
                '30秒サマリー',
                'CHARM試験',
                'CIBIS-II試験',
                '臨床的意義',
    ...
    async analyzeExistingContent() {
        ...
        // 新構造：drugs/とdrug_evolution/内のすべてのMDファイルを取得
        const allMdFiles = [
        ...
        for (const file of allMdFiles) {
            ...
            const fileType = this.categorizeContentFile(file, content);
            
            if (fileType.type === 'drugGroup') {
                // 薬効群モデルとして処理
                const analysis = this.analyzeDrugGroupContent(content, file);
                this.drugGroups.set(analysis.name, analysis);
                console.log(`  📚 薬効群モデル: ${analysis.name} - ${analysis.quality}級 (${analysis.score}/5.0)`);
            } else if (fileType.type === 'drug') {
                // 個別薬剤ファイルとして処理
                const analysis = this.analyzeIndividualDrugContent(content, file);
                this.individualDrugs.set(analysis.name, analysis);
                console.log(`  💊 個別薬剤: ${analysis.name} - ${analysis.lines}行`);
            }
        }

        // HTMLカード解析（新構造：study_guides/memory_tools/）
    ...
    async applyConversionStrategy() {
        ...
        // 2. 個別薬剤ファイルの処理
        console.log('\n💊 個別薬剤MDファイルの変換...');
        for (const [name, drugData] of this.individualDrugs) {
            await this.generateDrugDetailPage(drugData);
        }
    }
...
<body>
    ...
    saveHtmlFile(fileName, content, options = {}) {
        ...
        // 保護チェック（オプションで無効化可能）
        if (!options.forceOverwrite && this.shouldProtectExistingFile(filePath)) {
            const existingContent = fs.readFileSync(filePath, 'utf8');
            const existingLines = existingContent.split('\n').length;
            console.log(`  🛡️ 保護: ${fileName} (${existingLines}行の高品質ファイル)`);
            return;
        }
...
*このモデルは最新のエビデンスに基づく。PharmaDx品質基準5.0/5.0を満たす完全版。*`;
    ...
    categorizeContentFile(filePath, content) {
        ...
        // 1. ファイル名による判別
        if (this.isDrugGroupModelByName(filename)) {
            return { type: 'drugGroup', subtype: 'model' };
        }
        
        // 2. 内容による判別
        if (this.isIndividualDrugByContent(content)) {
     ...
     * ファイル名から薬効群モデルを判定
     */
    isDrugGroupModelByName(filename) {
        const modelPatterns = [
            '_evolution_model',
     ...
     * 内容から個別薬剤ファイルを判定
     */
    isIndividualDrugByContent(content) {
        const drugPatterns = [
            /^#\s+.+（.+）詳細解説モデル/m,
            /##\s+📍\s+5分サマリー/,
            /###\s+薬剤の本質/,
        ...
        const matchCount = drugPatterns.filter(pattern => pattern.test(content)).length;
        return matchCount >= 2;
    }
     ...
     * 個別薬剤コンテンツの分析
     */
    analyzeIndividualDrugContent(content, filePath) {
        const fileName = path.basename(filePath, '.md');
        const lines = content.split('\n');
        
        // タイトルから薬剤名を抽出
        const titleMatch = content.match(/^#\s+(.+?)（(.+?)）/m);
        const drugNameJp = titleMatch ? titleMatch[1] : fileName;
        const brandName = titleMatch ? titleMatch[2] : '';
        
        // 薬剤の本質を抽出
        const essenceMatch = content.match(/薬剤の本質[（(]1-2文で[）)][\s\S]*?\n(.+)/);
        const essence = essenceMatch ? essenceMatch[1] : '';
        ...
        return {
            ...
            essence,
            lines: lines.length,
            quality: lines.length > 500 ? 'A' : lines.length > 300 ? 'B' : 'C',
            hasHistory: content.includes('歴史的背景'),
            hasClinicalUse: content.includes('臨床使い分け'),
            hasSideEffects: content.includes('副作用'),
            hasFutureProspects: content.includes('将来展望')
        };
    }
    ...
    shouldProtectExistingFile(filePath) {
        ...
        // 100行以上のファイルは保護
        if (lines >= this.protectionSettings.minLines) {
            return true;
        }
        
        // 特定のマーカーを含むファイルは保護
        const hasDetailedContent = this.protectionSettings.detailedContentMarkers.some(
    ...
    /**
     * 個別薬剤MDファイルから詳細ページを生成
     */
    async generateDrugDetailPage(drugData) {
        console.log(`  🔄 ${drugData.drugNameJp}（${drugData.name}）- ${drugData.lines}行`);
        
        const content = drugData.content;
        
        // セクションごとに内容を抽出
        const sections = this.extractSectionsFromMd(content);
        
        // HTMLテンプレートの選択
        const template = drugData.lines > 500 ? 'detailed' : 'standard';
        
        // HTML生成
        const html = this.renderDrugDetailTemplate(template, {
            ...drugData,
            ...sections,
            generatedFrom: 'individual_md',
            lastUpdated: new Date().toISOString()
        });
        
        // 保存（保護機能付き）
        const outputPath = `drugs/${drugData.name}.html`;
     ...
     * MDファイルからセクションを抽出
     */
    extractSectionsFromMd(content) {
        const sections = {};
        
        // 5分サマリーの抽出
        const summaryMatch = content.match(/##\s+📍\s+5分サマリー([\s\S]*?)(?=##|$)/);
        if (summaryMatch) {
            sections.summary5min = marked(summaryMatch[1].trim());
        }
        
        // 歴史的背景の抽出
        const historyMatch = content.match(/##\s+🧬.*?歴史的背景([\s\S]*?)(?=##|$)/);
        if (historyMatch) {
            sections.history = marked(historyMatch[1].trim());
        }
        
        // 臨床使い分けの抽出
        const clinicalMatch = content.match(/##\s+💊.*?臨床使い分け([\s\S]*?)(?=##|$)/);
        if (clinicalMatch) {
            sections.clinicalUse = marked(clinicalMatch[1].trim());
        }
        
        // 副作用の抽出
        const sideEffectsMatch = content.match(/##\s+⚠️.*?副作用([\s\S]*?)(?=##|$)/);
        if (sideEffectsMatch) {
            sections.sideEffects = marked(sideEffectsMatch[1].trim());
        }
        
        // 将来展望の抽出
        const futureMatch = content.match(/##\s+🔮.*?将来展望([\s\S]*?)(?=##|$)/);
        if (futureMatch) {
            sections.futureProspects = marked(futureMatch[1].trim());
        }
    ...
    /**
     * 個別薬剤詳細ページのHTMLテンプレート
     */
    renderDrugDetailTemplate(templateType, data) {
        const drugInfo = config.drugInfo[data.name] || {};
        const category = drugInfo.category || '未分類';
...
<body>
    ...
    <nav class="breadcrumb">
        <div class="container">
            ...
            <span class="current">${data.drugNameJp}</span>
        </div>
    </nav>
    ...
    <main class="drug-detail">
        <div class="container">
            ...
            <section class="drug-header">
                <h1 class="drug-name">${data.drugNameJp}</h1>
                <div class="drug-subtitle">
                    <span class="brand-name">（${data.brandName}）</span>
                    <span class="drug-class">${category}</span>
                </div>
            </section>
            ...
            <section class="related-links">
                <h2>📚 関連コンテンツ</h2>
                <div class="link-grid">
                    <a href="../index.html#drugs" class="related-item">
                        薬剤一覧へ戻る
                    </a>
                    ${category !== '未分類' ? `
                    <a href="../groups/${category}.html" class="related-item">
                        ${category}の全体像を見る
                    </a>
                    ` : ''}
                </div>
    ...
    <footer class="footer">
        <div class="container">
            <p class="generated-info">
                Generated from: ${data.filePath}<br>
